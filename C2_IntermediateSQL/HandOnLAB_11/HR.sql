...
--
-- Host: labs-mysql-prehistoric-melodic-train:3306
-- Generation Time: Sep 16, 2025 at 07:16 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `EMPSALARY`  AS SELECT `EMPLOYEES`.`EMP_ID` AS `EMP_ID`, `EMPLOYEES`.`F_NAME` AS `F_NAME`, `EMPLOYEES`.`L_NAME` AS `L_NAME`, `EMPLOYEES`.`B_DATE` AS `B_DATE`, `EMPLOYEES`.`SEX` AS `SEX`, `JOBS`.`JOB_TITLE` AS `JOB_TITLE`, `JOBS`.`MIN_SALARY` AS `MIN_SALARY`, `JOBS`.`MAX_SALARY` AS `MAX_SALARY` FROM (`EMPLOYEES` join `JOBS`) WHERE (`EMPLOYEES`.`JOB_ID` = `JOBS`.`JOB_IDENT`) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `EMP_DEPT`  AS SELECT `D`.`DEP_NAME` AS `DEP_NAME`, `E`.`EMP_ID` AS `EMP_ID`, `E`.`F_NAME` AS `F_NAME`, `E`.`L_NAME` AS `L_NAME`, `E`.`DEP_ID` AS `DEP_ID` FROM (`EMPLOYEES` `E` join `DEPARTMENTS` `D`) WHERE (`E`.`DEP_ID` = `D`.`DEPT_ID_DEP`) ;
COMMIT;
